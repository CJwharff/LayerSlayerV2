...
document.addEventListener("input", (event) => {
  ...
  if (Number.isInteger(transientIndex) && transientKey && transientState.layers[transientIndex]) {
    ...
    if (transientKey === "initialTop" && transientState.layers[transientIndex + 1]) {
      const sibling = document.querySelector(
        `[data-transient-k="initialBottom"][data-transient-i="${transientIndex + 1}"]`
      );
      if (sibling) sibling.value = String(transientState.layers[transientIndex + 1].initialBottom);
    }
    if (transientKey === "initialBottom" && transientState.layers[transientIndex - 1]) {
      const sibling = document.querySelector(
        `[data-transient-k="initialTop"][data-transient-i="${transientIndex - 1}"]`
      );
      if (sibling) sibling.value = String(transientState.layers[transientIndex - 1].initialTop);
    }
    return;
  }
